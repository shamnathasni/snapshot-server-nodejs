    ...
    try {
        ...
        if (existOtp && otp == existOtp.otp ) {
            ...
            if (newUser) {
                ...
                res.json({ newUser, alert: "You have successfully signed up!", status: true, token });
            } else {
...
const  resendUserOtp = async (req, res )=>{
    try {
        ...
        const mailOption = {
            ...
            html:"<h3>OTP for account verification is </h3>" + "<h1 style='font-weight:bold;'>" +resendOTP.otp + "</h1>"
        }
        transporter.sendMail(mailOption)
...
const userLogin = async (req, res ) => {
    try {
        ...
        if (emailExist) {
            ...
            res.json({ newUser: emailExist,alert:"You have successfully signed in!", token, status: true });
            } else {
